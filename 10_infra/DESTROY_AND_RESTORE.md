...
aws rds describe-db-snapshots \
  ...
  --region ap-northeast-1 --profile terraform
```
...
```bash
# --- 停止（destroy）---
aws rds create-db-snapshot --db-instance-identifier tastylog-dev-mysql-standalone \
  --db-snapshot-identifier tastylog-dev-mysql-manual-YYYYMMDD --region ap-northeast-1 --profile terraform
aws rds wait db-snapshot-completed --db-snapshot-identifier tastylog-dev-mysql-manual-YYYYMMDD \
  --region ap-northeast-1 --profile terraform
terraform destroy
